"use client";

import { useState, useEffect, useRef } from "react";
import { Button } from "./button";
import { ChevronLeft, ChevronRight } from "lucide-react";
import { cn } from "../../lib/utils";
import { motion } from "framer-motion";

interface WheelPaginationProps {
  totalPages?: number;
  className?: string;
  visibleCount?: number; // Number of pages visible at once
  initialPage?: number; // Initial active page
  onChange?: (page: number) => void;
}

export default function WheelPagination({
  totalPages = 50,
  visibleCount = 5,
  className,
  initialPage = 0,
  onChange,
}: WheelPaginationProps) {
  const [active, setActive] = useState(initialPage);
  const containerRef = useRef<HTMLDivElement>(null);
  const onChangeRef = useRef(onChange);
  const prevActiveRef = useRef(active);

  // Keep onChange ref updated
  useEffect(() => {
    onChangeRef.current = onChange;
  }, [onChange]);

  // Update active page when initialPage changes
  useEffect(() => {
    setActive(initialPage);
    prevActiveRef.current = initialPage;
  }, [initialPage]);

  // Call onChange only when active actually changes (not on initial mount or initialPage change)
  useEffect(() => {
    if (active !== prevActiveRef.current && onChangeRef.current) {
      prevActiveRef.current = active;
      onChangeRef.current(active);
    }
  }, [active]);

  const prevPage = () => setActive((p) => Math.max(p - 1, 0));
  const nextPage = () => setActive((p) => Math.min(p + 1, totalPages - 1));

  const handleWheel = (e: WheelEvent) => {
    e.preventDefault();
    if (e.deltaY < 0) setActive((p) => Math.max(p - 1, 0));
    else if (e.deltaY > 0) setActive((p) => Math.min(p + 1, totalPages - 1));
  };

  useEffect(() => {
    const el = containerRef.current;
    if (!el) return;
    el.addEventListener("wheel", handleWheel, { passive: false });
    return () => el.removeEventListener("wheel", handleWheel);
  }, []);

  // Determine visible pages based on active
  const getVisiblePages = () => {
    const pages = [];
    const half = Math.floor(visibleCount / 2);
    let start = active - half;
    let end = active + half;

    if (start < 0) {
      end += -start;
      start = 0;
    }
    if (end > totalPages - 1) {
      start -= end - (totalPages - 1);
      end = totalPages - 1;
      if (start < 0) start = 0;
    }

    for (let i = start; i <= end; i++) pages.push(i);
    return pages;
  };

  const visiblePages = getVisiblePages();

  return (
    <div
      ref={containerRef}
      className={cn(
        "flex items-center gap-2 p-4 select-none cursor-pointer bg-white rounded-lg",
        className
      )}
    >
      {/* Previous arrow */}
      <Button
        variant="ghost"
        size="icon"
        onClick={prevPage}
        disabled={active === 0}
        className="text-gray-400 hover:text-primary disabled:opacity-40 transition-colors"
      >
        <ChevronLeft className="w-5 h-5" />
      </Button>

      {/* Page numbers carousel */}
      <div className="flex gap-2">
        {visiblePages.map((p) => (
          <motion.div
            key={p}
            layout
            animate={{ scale: active === p ? 1.3 : 1 }}
            transition={{ type: "spring", stiffness: 300, damping: 25 }}
            whileHover={{
              scale: active === p ? 1.35 : 1.15,
              transition: { 
                duration: 0.2,
                ease: "easeOut"
              }
            }}
            className={cn(
              "w-10 h-10 flex items-center justify-center rounded-full font-medium min-h-[40px] cursor-pointer transition-colors hover:animate-wiggle",
              active === p
                ? "bg-primary text-white border border-primary"
                : "bg-gray-100 text-gray-800 border border-gray-200 hover:bg-gray-200"
            )}
            onClick={() => setActive(p)}
          >
            {p + 1}
          </motion.div>
        ))}
      </div>

      {/* Next arrow */}
      <Button
        variant="ghost"
        size="icon"
        onClick={nextPage}
        disabled={active === totalPages - 1}
        className="text-gray-400 hover:text-primary disabled:opacity-40 transition-colors"
      >
        <ChevronRight className="w-5 h-5" />
      </Button>
    </div>
  );
}

