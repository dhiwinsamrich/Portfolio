      ...
      >
        <CardContent className="flex flex-col items-start gap-4 p-6 h-full">
          {isQuoteType ? (
            ...
            <>
              <p className="text-muted-foreground font-sans">"{testimonial.quote}"</p>
  ...
  return (
    ...
    >
      <div className="flex flex-col items-center text-center gap-4 mb-12">
        <h2 className="text-3xl md:text-4xl font-bold tracking-tighter font-serif">{title}</h2>
      </div>
      ...
      >
        {testimonials.map((testimonial, index) => {
          ...
          const key = testimonial.type === 'quote' 
            ? `quote-${testimonial.quote.substring(0, 20).replace(/\s+/g, '-')}-${index}` 
            : `user-${testimonial.name || 'anonymous'}-${testimonial.role || 'no-role'}-${index}`;
          return (
            <div key={key} className={cn(isMiddleItem && "md:col-span-2 lg:col-span-1")}>
