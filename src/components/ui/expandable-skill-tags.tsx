...
export const ExpandableSkillTags = ({
  ...
  return (
    <section className={cn("w-full", className)}>
      {title && (
        <h3 className="mb-4 text-lg font-semibold text-foreground" style={{ fontFamily: 'var(--font-sans)' }}>{title}</h3>
      )}
      <motion.div
      ...
      >
        ...
        {visibleSkills.map((skill, index) => (
          <motion.div key={`visible-${index}`} variants={itemVariants}>
            <Badge variant="secondary" className="text-xs sm:text-sm md:text-sm px-2.5 py-1 sm:px-3 sm:py-1.5 md:px-3.5 md:py-1.5">
              {skill}
            </Badge>
          </motion.div>
        ))}
